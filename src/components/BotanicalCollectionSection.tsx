...
const BotanicalCollectionSection = () => {
  ...
  useEffect(() => {
    const fetchBotanicalProducts = async () => {
      try {
        ...
        console.error("Error fetching botanical products:", error);
      } finally {
        setIsLoading(false);
      }
    };
    fetchBotanicalProducts();
  }, []);
  ...
  if (!isLoading && products.length === 0) {
    return null; // Don't render anything if no products in this category are found
  }
      ...
      <div className="container mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
            {products.map((product, i) => (
              <motion.div
                ...
                className="group cursor-pointer flex flex-col justify-between bg-zinc-50 border border-zinc-150 p-3 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
              >
                <div>
                  <Link 
                ...
                <div className="px-1 pt-3 mt-4 border-t border-zinc-150 flex items-center justify-between">
        ...
        {!isLoading && products.length > 0 && (
          <div className="text-center mt-12">
            <Link to="/shop?category=Floral%20%26%20Botanical" className="inline-block bg-white text-primary border border-zinc-200 hover:bg-primary hover:text-white hover:border-primary px-8 py-3 rounded-full text-xs font-bold tracking-[0.25em] uppercase transition-all duration-300 shadow-sm">
               View Full Botanical Collection
            </Link>
          </div>
        )}
      </div>
...
export default BotanicalCollectionSection;
